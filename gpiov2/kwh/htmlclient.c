...
const char *pagetemplate = "/ZWaveAPI/Run/devices[%d].instances[%d].commandClasses[%d].data[%d]";
...
int main(int argc, char **argv)
{
  ...
  sprintf(page, pagetemplate, 4, 1, 50, 2);
...
double getWatts(char *htmlPage)
{
	...
	{
		cJSON *wattsObject = cJSON_GetObjectItem(json,"val");
		double watts = cJSON_GetObjectItem(wattsObject, "value")->valuedouble; 
		cJSON_Delete(json);
		return watts;
	}
}
